...
mutserve(){
	...
	java -jar $mutserve call --reference $ref --write-raw --contig-name $contig_name --output $output $bam
}
...
merge_files(){
	...
	cat $output_dir/bc*/*_raw.txt | head -n1 >> $output_dir/$(basename $run_path)_summary_mutserve.txt
...
main(){
	for barcode in "$run_path"ont_pl/bc*; do
		echo $barcode
		get_bam $barcode
		get_ref $barcode
		get_contig $barcode
